...
export class WalletMonitorComparison {
  ...
  private async connectBirdeye(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        // Correct Birdeye WebSocket URL format with chain and API key
        const wsUrl = `wss://public-api.birdeye.so/socket/solana?x-api-key=${this.birdeyeApiKey}`;
        this.birdeyeWS = new WebSocket(wsUrl);
        ...
        this.birdeyeWS.on('message', (data: Buffer) => {
          try {
            const message = JSON.parse(data.toString());
            if (message.type === 'WALLET_TX') {
              this.handleBirdeyeTransaction(message.data);
            }
          } catch (error) {
